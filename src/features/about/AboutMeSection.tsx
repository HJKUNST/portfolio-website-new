...
import { useEffect, useRef, forwardRef, useState, type ReactNode } from "react";
...
const SectionSubtitle = ({ children, className = "" }: { children: ReactNode; className?: string }) => (
  <h3 className={`text-h3-em mb-3 ${className}`} style={subtitleStyle}>
    {children}
  </h3>
);

const SectionBody = ({ children, className = "" }: { children: ReactNode; className?: string }) => (
  <p className={`text-em ${className}`} style={BodyStyle}>
    {children}
  </p>
);

const ContentSection = forwardRef<HTMLDivElement, { title: string; children: ReactNode; className?: string }>(
